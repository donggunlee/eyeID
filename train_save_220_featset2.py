...
from model_save import StackedBRNN, supported_rnns
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        output_train = output_train.type(torch.cuda.FloatTensor)
        output_rnn_train = torch.tensor([])
        output_rnn_train = output_rnn_train.type(torch.cuda.FloatTensor)
        total_target_train = torch.tensor([])
        total_target_train = total_target_train.type(torch.cuda.FloatTensor)
        ...
        for i, (data) in enumerate(train_loader, start=start_iter):
            ...
            output_rnn, output = model(input, trial)

            output_train = torch.cat((output_train, output), 0)
            output_rnn_train = torch.cat((output_rnn_train, output_rnn), 0)
            ...
            total_target_train = torch.cat((total_target_train, target), 0)
            ...
            del loss
            del data, input, output_rnn, output, target, pred
        ...
        train_output_epoch_npy = output_train.cpu()
        train_output_epoch_npy = train_output_epoch_npy.detach().numpy()
        train_output_rnn_epoch_npy = output_rnn_train.cpu()
        train_output_rnn_epoch_npy = train_output_rnn_epoch_npy.detach().numpy()
        total_target_train_npy = total_target_train.cpu()
        total_target_train_npy = total_target_train_npy.detach().numpy()
        del output_train, output_rnn_train, total_target_train
        # del train_output_epoch_npy, train_output_rnn_epoch_npy, total_target_train_npy
        ...
        output_valid = output_valid.type(torch.cuda.FloatTensor)
        output_rnn_valid = torch.tensor([])
        output_rnn_valid = output_rnn_valid.type(torch.cuda.FloatTensor)
        ...
        for i, (data) in enumerate(valid_loader, start=start_iter):
            ...
            with torch.no_grad():
                # Forward
                output_rnn, output = model(input, trial)
                output_valid = torch.cat((output_valid, output), 0)
                output_rnn_valid = torch.cat((output_rnn_valid, output_rnn), 0)
        ...
        valid_output_epoch_npy = output_valid.cpu()
        valid_output_epoch_npy = valid_output_epoch_npy.detach().numpy()
        valid_output_rnn_epoch_npy = output_rnn_valid.cpu()
        valid_output_rnn_epoch_npy = valid_output_rnn_epoch_npy.detach().numpy()
        del output_valid, output_rnn_valid
        # del valid_output_epoch_npy, valid_output_rnn_epoch_npy
        ...
        output_test = output_test.type(torch.cuda.FloatTensor)
        output_rnn_test = torch.tensor([])
        output_rnn_test = output_rnn_test.type(torch.cuda.FloatTensor)
        # for i, (data) in tqdm(enumerate(valid_loader), total=len(valid_loader)):
        for i, (data) in enumerate(test_loader, start=start_iter):
            ...
            with torch.no_grad():
                # Forward
                output_rnn, output = model(input, trial)
                output_test = torch.cat((output_test, output), 0)
                output_rnn_test = torch.cat((output_rnn_test, output_rnn), 0)
            ...
            del output_rnn, output, target, pred
        ...
        test_output_epoch_npy = output_test.cpu()
        test_output_epoch_npy = test_output_epoch_npy.detach().numpy()
        test_output_rnn_epoch_npy = output_rnn_test.cpu()
        test_output_rnn_epoch_npy = test_output_rnn_epoch_npy.detach().numpy()
        del output_test, output_rnn_test
        ...
        matfilename = 'exp_220_featset2n_RNN' + str(args.rnn_layers) + '_' + str(args.rnn_hidden) + '_FC' + str(args.fc_layers) + '_' + str(args.fc_hidden)
        ...
        matfilename_output = 'exp_220_featset2n_RNN' + str(args.rnn_layers) + '_' + str(args.rnn_hidden) + '_FC' + str(args.fc_layers) + '_' + str(args.fc_hidden) + '_' + str(epoch)
        sio.savemat(matfilename_output, dict(train_target=total_target_train_npy, train_output=train_output_epoch_npy, train_output_rnn=train_output_rnn_epoch_npy, valid_output=valid_output_epoch_npy, valid_output_rnn=valid_output_rnn_epoch_npy, test_output=test_output_epoch_npy, test_output_rnn=test_output_rnn_epoch_npy))
        del total_target_train_npy, train_output_epoch_npy, train_output_rnn_epoch_npy, valid_output_epoch_npy, valid_output_rnn_epoch_npy, test_output_epoch_npy, test_output_rnn_epoch_npy